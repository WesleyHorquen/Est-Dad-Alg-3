...
bool excluirChave(PONT *raiz, TIPOCHAVE ch){
    ...
    if((!atual->esq)||(!atual->dir)){ //tem 0 ou 1 filho
        if(!atual->esq){
...
int main(){
    ...
    printf("\n\t> Exibindo arvore com parenteses:\n");
    exibirArvore(raiz);
    printf("\n");

    TIPOCHAVE chaveParaBuscar = 40;
    PONT resultadoBusca = buscaBinaria(chaveParaBuscar, raiz);
    if(resultadoBusca){
        printf("\n> Chave %d encontrada na arvore.\n", chaveParaBuscar);
    }else{
        printf("> Chave %d nao encontrada na arvore.\n", chaveParaBuscar);
    }

    TIPOCHAVE chaveParaExcluir = 20;
    if(excluirChave(&raiz, chaveParaExcluir)){
        printf("\n> Chave %d excluida com sucesso.\n", chaveParaExcluir);
    }else{
        printf("\n> Chave %d nao encontrada para exclusao.\n", chaveParaExcluir);
    }

    printf("\n\t> Arvore em ordem apos exclusao:\n");
    exibirArvoreEmOrdem(raiz);
    printf("\n\n");
